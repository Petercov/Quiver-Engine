...
int CNPC_PlayerCompanion::TranslateSchedule( int scheduleType ) 
{
	...
	case SCHED_RANGE_ATTACK1:
		...
		if (GetActiveWeapon() && IsRPG(GetActiveWeapon()))
		{
			...
			{
				if (OccupyStrategySlotRange(SQUAD_SLOT_PC_RPG1, SQUAD_SLOT_PC_RPG2))
				{
					return SCHED_PC_STRIDER_RANGE_ATTACK1_RPG;
				}
				else
				{
			...
			else
			{
				...
				if ((pPlayer && GetEnemy() && ((GetEnemy()->GetAbsOrigin() -
					...
					!OccupyStrategySlotRange(SQUAD_SLOT_PC_RPG1, SQUAD_SLOT_PC_RPG2))
				{
				...
				{
					return SCHED_PC_RANGE_ATTACK1_RPG;
				}
			}
		}
...
void CNPC_PlayerCompanion::EnterVehicle( CBaseEntity *pEntityVehicle, bool bImmediately )
{
	...
	CPropVehicleDriveable* pVehicle = dynamic_cast<CPropVehicleDriveable*>(pEntityVehicle);
...
#if defined(MAPBASE) || defined(HLSS_HE_GAME)
//-----------------------------------------------------------------------------
// 
//-----------------------------------------------------------------------------
...
AI_BEGIN_CUSTOM_NPC( player_companion_base, CNPC_PlayerCompanion )
	...
	DECLARE_TASK(TASK_PC_RPG_AUGER)
#ifdef MAPBASE
