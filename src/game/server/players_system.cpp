...
CPlayer *CPlayersSystem::GetRandom( int team )
{
    ...
    CPlayer *pPlayer = (UTIL_PlayerByIndex( players.Element(random) ));
    return pPlayer;
}
...
StatType CPlayersSystem::GetAmmoStats( int team )
{
    ...
    FOR_EACH_PLAYER_TEAM(
    {
        ...
        if ( ammo >= Ceil2Int(maxClip*0.2) )
            ++individual;

        // Más de la mitad - STATS_NORMAL
        if ( ammo >= Ceil2Int(maxClip*0.4) )
            ++individual;

        // Más de la mitad - STATS_GOOD
        if ( ammo >= Ceil2Int(maxClip*0.6) )
            ++individual;

        // Mucha - STATS_EXCELENT
        if ( ammo >= Ceil2Int(maxClip*0.8) )
            ++individual;
    ...
    // STATS_POOR
    if ( mediocreCount <= Ceil2Int(total*0.3) )
        ++type;

    // STATS_NORMAL
    if ( poorCount <= Ceil2Int(total*0.3) )
        ++type;

    // STATS_GOOD
    if ( normalCount >= Ceil2Int(total*0.3) )
        ++type;

    // STATS_EXCELENT
    if ( goodCount >= Ceil2Int(total*0.3) )
        ++type;
...
StatType CPlayersSystem::GetPlayersStats()
{
    ...
    if ( dejected >= Ceil2Int(total * 0.3f) )
        --type;

    // Jugadores incapacitados!
    if ( dejected >= Ceil2Int(total * 0.6f) )
        --type;
...
bool CPlayersSystem::IsAbleToSee( CBaseEntity *pEntity, const CRecipientFilter &filter, CBaseCombatCharacter::FieldOfViewCheckType checkFOV )
{
    for ( int i = 0; i < filter.GetRecipientCount(); ++i ) {
        CPlayer *pPlayer = UTIL_PlayerByIndex( filter.GetRecipientIndex( i ) );
...
bool CPlayersSystem::IsAbleToSee(const Vector & vecPosition, CBaseCombatCharacter::FieldOfViewCheckType checkFOV, bool ignoreBots, int team)
{
	...
	//	return true;
    if (pPlayer->FVisible(vecPosition) && !pPlayer->IsHiddenByFog(vecPosition) && (checkFOV == CBaseCombatCharacter::FieldOfViewCheckType::DISREGARD_FOV || pPlayer->FInViewCone(vecPosition)))
        return true;
		});
...
bool CPlayersSystem::IsEyesVisible( const Vector &vecPosition, bool ignoreBots, int team )
{
    FOR_EACH_PLAYER(
        ...
        // Es visible!
        if ( pPlayer->FVisible(vecPosition, MASK_VISIBLE) )
            return true;
    );
