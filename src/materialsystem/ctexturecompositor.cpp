...
enum { Neutral = 0, Red = 2, Blue = 3, Green = 4, Yellow = 5 };
...
public:
	...
	void ComputeRandomValues( int* pCurIndex, IUniformRandomStream* pRNGs, int nRNGCount );
...
protected:
	...
	virtual bool ComputeRandomValuesThis(IUniformRandomStream* pRNG ) = 0;
...
struct TextureStageParameters
{
	...
	CUtlString m_pTexGreenFilename;
	CUtlString m_pTexYellowFilename;
	Range m_AdjustBlack;
...
const ParseTableEntry cTextureStageParametersParseTable[] = 
{
	...
	{ "texture_green",		ParseStringFromKV,				offsetof(TextureStageParameters, m_pTexGreenFilename) },
	{ "texture_yellow",		ParseStringFromKV,				offsetof(TextureStageParameters, m_pTexYellowFilename) },

	{ "texture_team1",		ParseStringFromKV,				offsetof(TextureStageParameters, m_pTexRedFilename) },
	{ "texture_team2",		ParseStringFromKV,				offsetof(TextureStageParameters, m_pTexBlueFilename) },
	{ "texture_team3",		ParseStringFromKV,				offsetof(TextureStageParameters, m_pTexGreenFilename) },
	{ "texture_team4",		ParseStringFromKV,				offsetof(TextureStageParameters, m_pTexYellowFilename) },
...
public:
	...
	virtual ~CTCTextureStage()
	{
		...
		SafeRelease(&m_pTexGreen);
		SafeRelease(&m_pTexYellow);
	}
	...
	{ 
		...
		case Blue:
			...
			break;
		case Green:
			SafeAssign(&m_pTexGreen, pTex);
			break;
		case Yellow:
			SafeAssign(&m_pTexYellow, pTex);
			break;
		default:
...
protected:
	bool AreTexturesLoaded() const
	{
		...
		if (!m_Parameters.m_pTexGreenFilename.IsEmpty() && !m_pTexGreen)
			return false;

		if (!m_Parameters.m_pTexYellowFilename.IsEmpty() && !m_pTexYellow)
			return false;
	...
	ITexture* GetTeamSpecificTexture( int nTeam )
	{
		...
		if (nTeam == Green && m_pTexGreen)
			return m_pTexGreen;

		if (nTeam == Yellow && m_pTexYellow)
			return m_pTexYellow;
	...
	virtual void RequestTextures()
	{
		...
		if (!m_Parameters.m_pTexGreenFilename.IsEmpty())
			materials->AsyncFindTexture(m_Parameters.m_pTexGreenFilename.Get(), TEXTURE_GROUP_RUNTIME_COMPOSITE, this, (void*)Green, false, TEXTUREFLAGS_IMMEDIATE_CLEANUP);
		if (!m_Parameters.m_pTexYellowFilename.IsEmpty())
			materials->AsyncFindTexture(m_Parameters.m_pTexYellowFilename.Get(), TEXTURE_GROUP_RUNTIME_COMPOSITE, this, (void*)Yellow, false, TEXTUREFLAGS_IMMEDIATE_CLEANUP);
	}
	...
	virtual void ResolveThis( CTextureCompositor* _comp )
	{
		...
		if ( !m_pTex && !m_pTexRed && !m_pTexBlue && !m_pTexGreen && !m_pTexYellow )
		{
			_comp->Error( false, "Invalid texture_lookup node, must specify at least texture (or texture_red/team1 and texture_blue/team2 and texture_green/team3 and texture_yellow/team4) or all of them.\n" );
			return;
		}
		...
		{
			_comp->Error( false, "Failed to load texture_red/team1 '%s', this is non-recoverable.\n", m_Parameters.m_pTexRedFilename.Get() );
			return;
		}

		if ( m_pTexBlue && m_pTexBlue->IsError() )
		{
			_comp->Error( false, "Failed to load texture_blue/team2 '%s', this is non-recoverable.\n", m_Parameters.m_pTexBlueFilename.Get() );
			return;
		}

		if (m_pTexGreen && m_pTexGreen->IsError())
		{
			_comp->Error(false, "Failed to load texture_green/team3 '%s', this is non-recoverable.\n", m_Parameters.m_pTexGreenFilename.Get());
			return;
		}

		if (m_pTexYellow && m_pTexYellow->IsError())
		{
			_comp->Error(false, "Failed to load texture_yellow/team4 '%s', this is non-recoverable.\n", m_Parameters.m_pTexYellowFilename.Get());
			return;
		}
	...
	virtual bool ComputeRandomValuesThis(IUniformRandomStream* pRNG ) OVERRIDE
	{
...
private:
	...
	ITexture* m_pTexRed;
	ITexture* m_pTexBlue;
	ITexture* m_pTexGreen;
	ITexture* m_pTexYellow;
...
// ------------------------------------------------------------------------------------------------
void CTCStage::ComputeRandomValues( int* pCurIndex, IUniformRandomStream* pRNGs, int nRNGCount )
{
